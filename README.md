...
Após salvar a **configuração**, limpe o cache (`php8.1 bin/magento cache:flush`) — a homepage fica
no full page cache. Edições no **produto** (preço, nome, imagem) não precisam de flush: o bloco
expõe as cache identities do produto (`IdentityInterface`), então salvar o produto purga a
homepage automaticamente.
...
- **Robustez do frontend**: as chamadas usam `mage/storage` (a lib REST do Luma, não jQuery cru);
  abas ocultas pausam o polling (`visibilitychange`) e refazem o fetch ao voltar a ficar visíveis;
  uma requisição em andamento bloqueia o próximo tick (sem empilhamento); após 3 falhas sem valor
  para exibir, o badge é ocultado em vez de girar em "consultando" para sempre; o contêiner usa
  `aria-live="polite"` para leitores de tela anunciarem a mudança de estoque.
...
├── Model/GetStockUpdate.php              # qty salable via MSI, com micro-cache por token
├── Model/StockVersion.php                # token de mudança por SKU, em cache
├── Model/Data/StockUpdate.php            # DTO imutável da resposta
├── Plugin/                               # 304 condicional (ETag) + invalidação do token
...
│   └── web/
│       ├── js/view/stock.js              # componente Knockout com polling condicional (ETag)
│       ├── template/stock.html           # template KO do badge de estoque
│       └── css/source/_module.less
├── i18n/pt_BR.csv
├── composer.json                         # instalável via Composer (type magento2-module)
└── Test/Unit/                            # service, token, plugins e view model
```
