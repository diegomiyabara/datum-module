...
```bash
# o módulo já está em app/code/Miyabara/FeaturedProduct
...
- **Endpoint**: `GET /rest/V1/featured-product/salable-qty?version=<token>` — service contract
  (`Api/GetSalableQtyInterface`) exposto via `webapi.xml` com acesso anônimo. Retorna a quantidade
...
- **Polling barato (token de versão)**: dois plugins `after` invalidam um token em cache quando o
  estoque pode ter mudado. O token vive num **cache customizado declarado por virtual type** —
  ...
  A qty só é recalculada quando o token está defasado.
...
├── Api/GetSalableQtyInterface.php        # contrato do endpoint de estoque
...
├── Model/GetSalableQty.php               # qty salable via MSI, com curto-circuito por token
...
├── i18n/pt_BR.csv
└── Test/Unit/Model/GetSalableQtyTest.php
```
